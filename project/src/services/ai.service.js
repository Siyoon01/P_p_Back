...
async function requestRecipeRecommendation(params) {
  ...
  try {
    logger.info('[AI:recsys]', `레시피 추천 요청 시작: userId=${params.userId}, candidates=${params.candidates?.length || 0}개, selectedIngredients=${params.query?.selectedIngredientIds?.length || 0}개`);
    ...
    logger.info('[AI:recsys]', `레시피 추천 완료 (소요시간: ${elapsedTime}ms, 추천 개수: ${recommendationCount}개)`);
    return result;
  } catch (error) {
    const elapsedTime = Date.now() - startTime;
    logger.error('[AI:recsys]', `레시피 추천 요청 실패 (소요시간: ${elapsedTime}ms): ${error.message}`);
    throw {
