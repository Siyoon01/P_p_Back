 ...
 * 특정 사용자 정보 (닉네임, 이름, 성별, 생년월일, 알레르기, 조리도구)를 수정합니다.
 */
const updateUserById = async (req, res) => {
    try {
        ...
        console.log(`[Admin] 사용자 ${userId} 정보 수정 요청:`, JSON.stringify(updateData, null, 2));
        
        // 1. 기본 정보 수정 (nickname, fullName, birthdate, gender)
        ...
        const updatedUser = await UserRepository.adminUpdate(userId, basicUpdateData);

        // 2. 알레르기 정보 업데이트 (항상 실행 - 필드가 있으면)
        if ('allergies' in updateData) {
            console.log(`[Admin] 사용자 ${userId} 알레르기 업데이트 시작`);
            console.log(`[Admin] 받은 알레르기 데이터:`, updateData.allergies);
            console.log(`[Admin] 데이터 타입:`, typeof updateData.allergies, Array.isArray(updateData.allergies));
            ...
            try {
                // 빈 배열도 허용 (모든 알레르기 제거)
                console.log(`[Admin] AllergyRepository.updateUserAllergies 호출 전`);
                await AllergyRepository.updateUserAllergies(userId, updateData.allergies);
                console.log(`[Admin] 사용자 ${userId} 알레르기 업데이트 성공`);
            } catch (allergyError) {
                console.error(`[Admin] 사용자 ${userId} 알레르기 업데이트 실패:`, allergyError);
                console.error(`[Admin] 알레르기 에러 상세:`, allergyError.message);
                console.error(`[Admin] 알레르기 에러 스택:`, allergyError.stack);
                throw allergyError;
            }
        } else {
            console.log(`[Admin] 사용자 ${userId} 알레르기 필드가 요청에 없음`);
        }

        // 3. 조리도구 정보 업데이트 (항상 실행 - 필드가 있으면)
        if ('tools' in updateData) {
            console.log(`[Admin] 사용자 ${userId} 조리도구 업데이트 시작`);
            console.log(`[Admin] 받은 조리도구 데이터:`, updateData.tools);
            console.log(`[Admin] 데이터 타입:`, typeof updateData.tools);
            
            if (!updateData.tools || typeof updateData.tools !== 'object' || Array.isArray(updateData.tools)) {
                return res.status(400).json({
                    success: false,
                    result_code: 400,
            ...
            // 객체를 배열로 변환: 값이 true인 key만 추출
            const toolNames = Object.keys(updateData.tools).filter(key => updateData.tools[key] === true);
            console.log(`[Admin] 추출된 조리도구 이름 배열:`, toolNames);
            
            try {
                // 빈 배열도 허용 (모든 조리도구 제거)
                console.log(`[Admin] ToolRepository.updateUserTools 호출 전`);
                await ToolRepository.updateUserTools(userId, toolNames);
                console.log(`[Admin] 사용자 ${userId} 조리도구 업데이트 성공`);
            } catch (toolError) {
                console.error(`[Admin] 사용자 ${userId} 조리도구 업데이트 실패:`, toolError);
                console.error(`[Admin] 조리도구 에러 상세:`, toolError.message);
                console.error(`[Admin] 조리도구 에러 스택:`, toolError.stack);
                throw toolError;
            }
        } else {
            console.log(`[Admin] 사용자 ${userId} 조리도구 필드가 요청에 없음`);
        }
 ...
 * 레시피 통계 (조회수 기준 상위 10개)를 조회합니다.
 */
const getRecipeStats = async (req, res) => {
    try {
        // 실제 DB에서 view_count 기준 상위 10개 레시피 조회
        const topRecipes = await RecipeRepository.findTopViewedRecipes(10);
        
        // 프론트엔드에서 요구하는 형식으로 변환 (name, value)
        const stats = topRecipes.map(recipe => ({
            name: recipe.title,
            value: recipe.view_count || 0 // view_count가 없으면 0
        }));
