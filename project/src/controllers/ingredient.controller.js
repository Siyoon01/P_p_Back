...
const validateIngredientInput = (data) => {
    ...
    if (!name || !expiryDate || quantity_value === undefined || quantity_value === null || quantity_value <= 0 || !quantity_unit) {
        const error = new Error('식재료 이름, 소비기한, 수량, 단위를 모두 입력해주세요. 수량은 0보다 큰 값이어야 합니다.');
