...
const getRecommendedRecipes = async (req, res) => {
    try {
        ...
        logger.info(`[User:${userId}]`, `POST /api/recipes/recommend 요청 수신 (ingredients: ${ingredients}, theme: ${theme}, requireMain: ${requireMain})`);
        ...
        // selectedIngredientsIds가 문자열 배열(재료 이름)인 경우 ingredient_master_id로 변환
        let selectedIngredientMasterIds = query.selectedIngredientsIds;
        if (selectedIngredientMasterIds.length > 0 && typeof selectedIngredientMasterIds[0] === 'string') {
            // 재료 이름 배열을 ingredient_master_id 배열로 변환
            const { IngredientMasterRepository } = require('../models/IngredientMaster.model');
            const ingredientNames = selectedIngredientMasterIds;
            const ingredientMasters = await IngredientMasterRepository.findByNames(ingredientNames);
            selectedIngredientMasterIds = ingredientMasters.map(im => im.id);
            logger.info(`[User:${userId}]`, `재료 이름 ${ingredientNames.length}개를 ID ${selectedIngredientMasterIds.length}개로 변환`);
        }
        ...
        logger.info(`[User:${userId}]`, `필터링 시작 (알레르기: ${userAllergyIds.length}개, 도구: ${userToolIds.length}개, 보유재료: ${ownedIngredientMasterIds.length}개)`);
        ...
        logger.info(`[User:${userId}]`, `필터링 후 candidates 수: ${candidates.length}개`);
        ...
        // 3. AI 서버 호출
        logger.info(`[User:${userId}]`, `AI 서버 호출 시작 (candidates: ${candidates.length}개)`);
        ...
        try {
            aiResponse = await aiService.requestRecipeRecommendation({
                ...
                query: {
                    ...
                    selectedIngredientIds: selectedIngredientMasterIds
                },
                requireMain: requireMain || false,
                candidates: candidates
            });
            const aiElapsedTime = Date.now() - aiStartTime;
            logger.info(`[User:${userId}]`, `AI 서버 호출 완료 (소요시간: ${aiElapsedTime}ms, 추천 개수: ${aiResponse.recommendations?.length || 0}개)`);
        } catch (error) {
            // AI 서버 통신 실패 처리
            const aiElapsedTime = Date.now() - aiStartTime;
            logger.error(`[User:${userId}]`, `POST /api/recipes/recommend - AI 서버 통신 실패 (소요시간: ${aiElapsedTime}ms): ${error.message}`);
        ...
        if (recommendedRecipeIds.length === 0) {
            logger.info(`[User:${userId}]`, 'POST /api/recipes/recommend - 추천 결과 없음');
            return res.status(200).json({
                success: true,
                result_code: 200,
                message: '맞춤 레시피 추천 성공',
        ...
        // 5. 레시피 상세 정보 조회
        logger.info(`[User:${userId}]`, `레시피 상세 정보 조회 시작 (recipeIds: ${recommendedRecipeIds.join(', ')})`);
        const recommendedRecipes = await RecipeRepository.getRecipesByIds(recommendedRecipeIds);
        logger.info(`[User:${userId}]`, `레시피 상세 정보 ${recommendedRecipes.length}건 조회 완료`);
        ...
        try {
            ...
            logger.info(`[User:${userId}]`, `추천 로그 ${recommendedRecipeIds.length}건 기록 완료`);
        } catch (logError) {
            logger.error(`[User:${userId}]`, `추천 로그 기록 실패: ${logError.message}`);
        }

        logger.info(`[User:${userId}]`, `POST /api/recipes/recommend 완료 - 레시피 ${formattedRecipes.length}건 추천 성공 (IDs: ${recommendedRecipeIds.join(', ')})`);
        res.status(200).json({
            success: true,
            result_code: 200,
            message: '맞춤 레시피 추천 성공',
...
const getRecipeHistory = async (req, res) => {
    try {
        ...
        if (!userId) {
            logger.warn(`[User:unknown]`, 'GET /api/recipes/history - userId가 없습니다.');
            return res.status(400).json({
                success: false,
                result_code: 400,
        ...
        // 항상 배열로 반환 (null이나 undefined 방지)
        const safeData = Array.isArray(historyRecipes) ? historyRecipes : [];
        
        res.status(200).json({
            success: true,
            result_code: 200,
            message: '최근 본 레시피 목록입니다.',
            data: safeData // Array of enriched recipe objects
        });
    } catch (error) {
        logger.error(`[User:${req.user?.id || 'unknown'}]`, `레시피 조회 기록 불러오기 중 오류: ${error.message}`);
        logger.error(`[User:${req.user?.id || 'unknown'}]`, `에러 스택: ${error.stack}`);
        res.status(500).json({
            success: false,
            result_code: 500,
...
const createRecipeViewLog = async (req, res) => {
    try {
        ...
        if (!userId) {
            logger.warn('[User:unknown]', 'POST /api/recipes/:id/view - userId가 없습니다.');
            return res.status(400).json({
                success: false,
                result_code: 400,
                message: '사용자 정보가 없습니다.'
        ...
        if (isNaN(recipeId) || recipeId <= 0) {
            logger.warn(`[User:${userId}]`, `POST /api/recipes/:id/view - 잘못된 recipeId: ${req.params.id}`);
            return res.status(400).json({
                success: false,
                result_code: 400,
        ...
        logger.info(`[User:${userId}]`, `POST /api/recipes/${recipeId}/view 요청 수신`);
        
        // 1. 조회 기록 로그 테이블에 기록
        await RecipeViewLogRepository.createLog(userId, recipeId);
        logger.info(`[User:${userId}]`, `레시피 ${recipeId} 조회 로그 저장 완료`);
        
        // 2. 레시피의 전체 조회수 카운트를 증가
        await RecipeRepository.incrementViewCount(recipeId);
        logger.info(`[User:${userId}]`, `레시피 ${recipeId} 조회수 증가 완료`);
        ...
        logger.error(`[User:${req.user?.id || 'unknown'}]`, `조회 기록 저장 중 오류: ${error.message}`);
        logger.error(`[User:${req.user?.id || 'unknown'}]`, `에러 스택: ${error.stack}`);
        res.status(500).json({
            success: false,
            result_code: 500,
