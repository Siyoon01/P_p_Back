...
class ToolRepository {
  ...
  static async updateUserTools(userId, toolNames) {
    ...
    try {
      ...
      console.log(`[ToolRepository] 사용자 ${userId} 기존 조리도구 삭제 시작`);
      const [deleteResult] = await connection.execute('DELETE FROM user_tools WHERE user_id = ?', [userId]);
      console.log(`[ToolRepository] 삭제된 행 수:`, deleteResult.affectedRows);

      // 3. 새로운 조리도구 정보가 있다면, 처리
      if (toolNames.length > 0) {
        console.log(`[ToolRepository] 새로운 조리도구 ${toolNames.length}개 추가 시작`);
        // 3-1. 조리도구 이름으로 ID 배열 조회
        const toolIds = await this.findIdsByNames(toolNames, connection);
        console.log(`[ToolRepository] 조회된 조리도구 ID:`, toolIds);
        
        // 3-2. user_tools 테이블에 새로운 정보 삽입 (bulk insert)
        const values = toolIds.map(toolId => [userId, toolId]);
        console.log(`[ToolRepository] 삽입할 값:`, values);
        const [insertResult] = await connection.query('INSERT INTO user_tools (user_id, tool_id) VALUES ?', [values]);
        console.log(`[ToolRepository] 삽입된 행 수:`, insertResult.affectedRows);
      } else {
        console.log(`[ToolRepository] 새로운 조리도구 없음 (모두 제거됨)`);
      }

      await connection.commit(); // 모든 작업이 성공했으면 커밋
      console.log(`[ToolRepository] 트랜잭션 커밋 완료`);
