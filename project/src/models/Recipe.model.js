...
class Recipe {
    ...
    static async createDetailResponse(recipe, ingredients, steps, tools = []) {
        ...
        return {
            ...
            tools: tools.map(t => t.name)      // Array of tool names
        };
    }
}
...
class RecipeRepository {
    ...
    static async findById(recipeId) {
        try {
            ...
            const [toolRows] = await pool.execute(
                `SELECT t.name FROM recipe_tools rt 
                 JOIN tools t ON rt.tool_id = t.id
                 WHERE rt.recipe_id = ?`, [recipeId]);

            // 5. 결과 통합하여 API 응답 형식으로 반환
            return Recipe.createDetailResponse(recipe, ingRows, stepRows, toolRows);
    ...
    static async filterRecipesForRecommendation(userAllergyIds, userToolIds, ownedIngredientMasterIds, requireMain, queryText) {
        try {
            ...
            // 키워드는 AI 추천 점수 계산에만 사용하고, DB 필터링에서는 사용하지 않음
            
            // 1차 필터링: 알레르기 필터링
            // 알레르기 필터링: recipe_allergies와 user_allergies를 비교하여 사용자 알레르기가 레시피에 포함되면 제외
            if (userAllergyIds && userAllergyIds.length > 0) {
                const allergyPlaceholders = userAllergyIds.map(() => '?').join(',');
                query += `
                    AND r.id NOT IN (
                        SELECT DISTINCT ra.recipe_id 
                        FROM recipe_allergies ra
                        WHERE ra.allergy_id IN (${allergyPlaceholders})
                    )
                `;
                params.push(...userAllergyIds);
            }
    ...
    static async searchRecipes(keyword) { 
        try {
            ...
            const [ingRows] = await pool.execute(
                `SELECT ri.recipe_id, im.name, ri.quantity 
                 FROM recipe_ingredients ri 
                 JOIN ingredients_master im ON ri.ingredient_master_id = im.id
            ...
            ingRows.forEach(ing => {
                ...
                const ingredientText = ing.quantity 
                    ? `${ing.name} ${ing.quantity}`
                    : ing.name;
                ingredientsByRecipe[ing.recipe_id].push(ingredientText);
            });
            ...
            console.error('RecipeRepository.searchRecipes 오류:', error);
            throw error;
        }
    }
    
    /**
     * 관리자용: 모든 레시피 목록을 재료 정보와 함께 조회합니다.
     * @returns {Array<Object>} 레시피 목록 (id, title, main_image_url, ingredients)
     */
    static async findAllForAdmin() {
        try {
            // 1. 모든 레시피 기본 정보 조회
            const [recipeRows] = await pool.execute(
                'SELECT id, title, main_image_url FROM recipes ORDER BY id ASC'
            );
            ...
            console.error('RecipeRepository.findAllForAdmin 오류:', error);
            throw error;
        }
    }
    ...
    static async findTopViewedRecipes(limit = 10) {
        try {
            ...
            // 테스트 스크립트와 동일한 방식: pool.query 사용, LIMIT 직접 삽입
            const query1 = `SELECT id, title, description, main_image_url, COALESCE(view_count, 0) AS view_count
                           FROM recipes 
                           ORDER BY COALESCE(view_count, 0) DESC 
                           LIMIT ${limitNum}`;
            const [recipeRows] = await pool.query(query1);
            ...
            // 2. 레시피 ID 목록 추출
            const recipeIds = recipeRows.map(r => parseInt(r.id, 10));
            
            // 3. 각 레시피의 재료 목록 조회 (recipeIds가 있을 때만)
            // MySQL2 파라미터 타입 문제 해결: pool.query 사용 + 파라미터를 문자열로 캐스팅
            // .execute()는 LIMIT/OFFSET placeholder를 제대로 처리하지 못하므로 .query() 사용
            let ingredientsByRecipe = {};
            if (recipeIds.length > 0) {
                const placeholders = recipeIds.map(() => '?').join(',');
                const query2 = `SELECT ri.recipe_id, im.name 
                               FROM recipe_ingredients ri 
                               JOIN ingredients_master im ON ri.ingredient_master_id = im.id
                               WHERE ri.recipe_id IN (${placeholders})
                               ORDER BY ri.recipe_id, ri.ingredient_master_id`;
                // 파라미터를 문자열로 캐스팅하여 타입 문제 해결 + pool.query 사용
                const recipeIdsAsStrings = recipeIds.map(id => String(id));
                const [ingRows] = await pool.query(query2, recipeIdsAsStrings);
                
                // 4. 레시피별 재료 그룹화
                ingRows.forEach(ing => {
                    const recipeId = parseInt(ing.recipe_id, 10);
                    if (!ingredientsByRecipe[recipeId]) {
                        ingredientsByRecipe[recipeId] = [];
                    }
                    ingredientsByRecipe[recipeId].push(ing.name);
                });
            }
            ...
            return recipeRows.map(recipe => ({
                ...
                title: recipe.title,
                description: recipe.description || '',
                required_ingredients: ingredientsByRecipe[parseInt(recipe.id, 10)] || [],
                main_image_url: recipe.main_image_url || '',
                view_count: parseInt(recipe.view_count, 10) || 0
            }));
    ...
    static async findHistoryByUserId(userId) {
        try {
            ...
            if (isNaN(userIdNum) || userIdNum <= 0) {
                console.error('RecipeRepository.findHistoryByUserId: 잘못된 userId:', userId);
                return [];
            }
            
            // 1. 최근 조회한 레시피 ID 목록 조회 (중복 제거, 최신순)
            const recentViews = await RecipeViewLogRepository.findDistinctRecentViews(userIdNum, 50);
            
            if (!recentViews || recentViews.length === 0) {
                console.log(`RecipeRepository.findHistoryByUserId: 사용자 ${userIdNum}의 조회 기록이 없습니다.`);
                return [];
            }
            
            // 2. 레시피 ID 목록 추출
            const recipeIds = recentViews
                .map(view => {
                    const id = parseInt(view.id, 10);
                    return isNaN(id) ? null : id;
                })
                .filter(id => id !== null);
            
            if (recipeIds.length === 0) {
                console.warn('RecipeRepository.findHistoryByUserId: 유효한 레시피 ID가 없습니다.');
                return [];
            }
            ...
            if (!recipeRows || recipeRows.length === 0) {
                console.warn(`RecipeRepository.findHistoryByUserId: 레시피 정보를 찾을 수 없습니다. (IDs: ${recipeIds.join(', ')})`);
                return [];
            }
            ...
            // 최근 조회 순서대로 정렬 (존재하는 레시피만 반환)
            const result = recentViews
                .map(view => {
                    const viewId = parseInt(view.id, 10);
                    if (isNaN(viewId) || !recipeMap[viewId]) {
                        return null;
                    }
                    const recipe = recipeMap[viewId];
                    return {
                        id: viewId,
                        title: recipe.title || '',
                        description: recipe.description || '',
                        required_ingredients: ingredientsByRecipe[viewId] || [],
                        main_image_url: recipe.main_image_url || ''
                    };
                })
                .filter(item => item !== null);
            
            console.log(`RecipeRepository.findHistoryByUserId: 사용자 ${userIdNum}의 최근 본 레시피 ${result.length}개 반환`);
            return result;
