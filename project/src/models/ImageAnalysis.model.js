...
class ImageAnalysisRepository {
     ...
     */
    static async create({ userId, image_url, original_filename, status = 'pending' }) {
        try {
            // status를 소문자로 변환하여 저장 (enum: 'pending','processing','completed','failed')
            const normalizedStatus = status.toLowerCase();
            const query = `
                INSERT INTO image_analyses (user_id, image_url, original_filename, status) 
                VALUES (?, ?, ?, ?)
            `;
            const [result] = await pool.execute(query, [userId, image_url, original_filename, normalizedStatus]);
    ...
    static async updateStatus(id, newStatus) {
        try {
            ...
            await pool.execute(query, [normalizedStatus, id]);
        } catch (error) {
