...
class AllergyRepository {
  ...
  static async updateUserAllergies(userId, allergyNames) {
    ...
    try {
      ...
      console.log(`[AllergyRepository] 사용자 ${userId} 기존 알레르기 삭제 시작`);
      const [deleteResult] = await connection.execute('DELETE FROM user_allergies WHERE user_id = ?', [userId]);
      console.log(`[AllergyRepository] 삭제된 행 수:`, deleteResult.affectedRows);

      // 3. 새로운 알레르기 정보가 있다면, 처리
      if (allergyNames.length > 0) {
        console.log(`[AllergyRepository] 새로운 알레르기 ${allergyNames.length}개 추가 시작`);
        // 3-1. 알레르기 이름으로 ID 배열 조회
        const allergyIds = await this.findIdsByNames(allergyNames, connection);
        console.log(`[AllergyRepository] 조회된 알레르기 ID:`, allergyIds);
        
        // 3-2. user_allergies 테이블에 새로운 정보 삽입 (bulk insert)
        const values = allergyIds.map(allergyId => [userId, allergyId]);
        console.log(`[AllergyRepository] 삽입할 값:`, values);
        const [insertResult] = await connection.query('INSERT INTO user_allergies (user_id, allergy_id) VALUES ?', [values]);
        console.log(`[AllergyRepository] 삽입된 행 수:`, insertResult.affectedRows);
      } else {
        console.log(`[AllergyRepository] 새로운 알레르기 없음 (모두 제거됨)`);
      }

      await connection.commit(); // 모든 작업이 성공했으면 커밋
      console.log(`[AllergyRepository] 트랜잭션 커밋 완료`);
