...
class IngredientMasterRepository {
  ...
  /**
   * 이름 배열을 기준으로 마스터 재료 목록을 조회합니다.
   * @param {Array<string>} names - 재료 이름 배열
   * @returns {Promise<Array<IngredientMaster>>} 마스터 재료 배열
   */
  static async findByNames(names) {
    if (!names || names.length === 0) return [];
    
    const placeholders = names.map(() => '?').join(',');
    const query = `SELECT id, name FROM ingredients_master WHERE name IN (${placeholders})`;
    
    try {
      const [rows] = await pool.execute(query, names);
      return rows.map(row => new IngredientMaster(row));
    } catch (error) {
      console.error('IngredientMasterRepository.findByNames 오류:', error);
