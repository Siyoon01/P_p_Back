...
const logger = require('./src/utils/logger'); // 로거 초기화

const app = express();

// 서버 시작 로그
logger.info('[Server]', '서버 초기화 시작');
...
const frontendBuildPath = path.join(__dirname, '../recipe/build');
...
// 서버 시작
const PORT = process.env.PORT || 65031;
app.listen(PORT, () => {
  console.log(`🚀 서버가 포트 ${PORT}에서 실행 중입니다.`);
  console.log(`📡 API 엔드포인트: http://localhost:${PORT}/api`);
  logger.info('[Server]', `서버가 포트 ${PORT}에서 실행 중입니다.`);
  logger.info('[Server]', `API 엔드포인트: http://localhost:${PORT}/api`);
});
